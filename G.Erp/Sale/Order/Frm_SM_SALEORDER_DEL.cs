...
{
    public partial class Frm_SM_SALEORDER_DEL : G.APP.BaseForm.BaseEditor
    {
        public Frm_SM_SALEORDER_DEL()
        {
            InitializeComponent();
            InitInfo();
        }
        private void InitInfo()
        {
            ...
            this.listTool = new List<string>() { "打印","退出" };
            this.T = typeof(SM_SALEORDER_DEL);
        ...
        private EditorAdapter<SM_SALEORDER_DEL, SM_SALEORDERDTL_DEL> initAdapter()
        {
            EditorAdapter<SM_SALEORDER_DEL, SM_SALEORDERDTL_DEL> adapter = new EditorAdapter<SM_SALEORDER_DEL, SM_SALEORDERDTL_DEL>();
            ...
            adapter.AddEditorInfo(new EditorInfo() { FieldCaption = "订单明细", FieldName = "SALEORDERDTL_DEL", Editor = this.gGrid1, IsReadOnly = true });
            return adapter;
        }
            ...
            this.gToolBar1.ButtonClick = new Controls.GToolBar.GToolBar.FunButtonClick(name =>
            {
                switch (name)
                {
                    ...
                    case "打印":
                        try
                        {
                            //PrintHelper phelp = new PrintHelper(@"\Sale\SALEORDER2.frx", new List<string>() { "SM_SALEORDER", "SM_SALEORDERDTL" }, null);
                            //phelp.PrintEntity = new List<SM_SALEORDER>() { this.editData as SM_SALEORDER };
                            //phelp.PrintByEntity(false);
                        }
                        catch (Exception e)
                        {
