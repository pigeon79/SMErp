...
namespace G.Data.Model
{
    public class ObjectInfo
    {
        ...
        public MemberHandler GetBelongsTo(Type t)
        {
        ...
        public MemberHandler GetHasAndBelongsToMany(Type t)
        {
            foreach (MemberHandler member in this.RelationMembers)
            {
        ...
        internal static FromClause GetObjectFromClause(Type modelType)
        {
            ...
            var attributev=modelType.GetAttribute<DbViewAttribute>(false);
            ...
            if (attribute == null && attributev==null)
            {
                return GetJoinedFrom(modelType, attributes);
            }
            if (attribute == null && attributev != null)
            {
                return new FromClause(modelType.Name, attributev.ViewName);
            }
            if (attribute.TableName == null)
            {
                return new FromClause(attribute.PartOf);
            }
            return new FromClause(GetTableNameFromConfig(attribute.TableName),attributev.ViewName);
        }
        ...
        {
            return ConfigHelper.GSettings.GetValue("@" + definedName, definedName);
        }
