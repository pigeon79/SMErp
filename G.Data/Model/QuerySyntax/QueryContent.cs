...
namespace G.Data.Model.QuerySyntax
{
    [Serializable]
    public class QueryContent<T> : IWhere<T>, IAfterWhere<T>, IRangeable<T>, IUpdatable<T>, IGetPagedSelector<T> where T : class, IDbObject, new()
    {
        ...
        public QueryContent(FromClause from,ModelContext ctx)
        {
            _ctx = ctx;
            this._from = from;
        }
        ...
        private List<TResult> InnerSelect<TResult>(bool distinct, bool noLazy)
        {
            ...
            _ctx.Operator.FillCollection(ret, typeof(TResult), _from, _where, _order, _range, distinct, noLazy);
            return ret;
        }
        ...
        {
            var smt = _ctx.Composer.GetSelectStatementBuilder(_from, _where, _order, 
