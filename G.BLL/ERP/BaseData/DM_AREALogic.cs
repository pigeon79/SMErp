...
{
    public class DM_AREALogic : BaseBLL<DM_AREA>, G.Interface.BLL.ISearch
    {
        public System.Data.DataTable SearchValue(Condition SearchCondition, string SearchText, string Fields, string ValueField)
        {
            ...
            if (!String.IsNullOrEmpty(SearchText))
            {
                ...
                {
                    c = CK.K["DQDM"].LeftLike(SearchText);
                    c |= CK.K["DQNAME"].Like(SearchText);
                }
                else
                {
            ...
            return this.ExecuteSelect(c, OrderBy.Parse("DQNAME"), 100, 0, ref allcount).ToDataTable();
        }
    }
}
